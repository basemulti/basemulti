...
export default function HasOneOrManyEditor({
  baseId,
  hasMany = false,
}: {
  baseId: string;
  ...
  const defaultForeignKey = schema?.getDefaultForeignKey(tableName);
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {field.value && <TableIconSelector size="sm" baseId={baseId} tableName={field.value} selector={false} />}
                      ...
                      {tables && Object.entries(tables).map(([relationTableName, relationTableSchema]) => (
                        <CommandItem
                          ...
                          onSelect={() => {
                            ...
                            setValue('name', snakeCase(
                              hasMany ? pluralize(relationTableName) : pluralize.singular(relationTableName)
                            ));
                            setOpen(false);
                          }}
                        >
    ...
    <div className="flex items-end gap-2">
      <FormField
        ...
        render={({ field }) => {
          ...
          const foreignFields = table ? schema?.getFields(table) : {};

          return (
            <FormItem className="mb-0 lg:mb-0 flex-1">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <FormLabel>{tables?.[table]?.label || t('foreign_key')}</FormLabel>
                <FormControl>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {foreignFields?.[field.value]
                            ? field.value
                            : <div className="text-muted-foreground">{schema?.hasField(table, defaultForeignKey as string) ? defaultForeignKey : t('select_field_prompt')}</div>}
                        </div>
                        <CaretSortIcon className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </PopoverTrigger>
      ...
      <FormField
        ...
        render={({ field }) => {
          ...
          return (
            <FormItem className="mb-0 lg:mb-0 flex-1">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <FormLabel>{tables?.[tableName]?.label || 'Local Key'}</FormLabel>
