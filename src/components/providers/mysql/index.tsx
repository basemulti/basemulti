...
export function Icon({ className }: IconProps) {
  return <img className={cn("size-5", className)} src="/mysql.svg" />;
}
...
export function ConnectionEditor({ title }: ConnectionEditorProps) {
  ...
  const handleImportConnection = () => {
    ...
    setValue('port', connectionObject.port ?? 3306);
          ...
          render={({ field }) => (
            <FormItem>
              <div className="grid w-full max-w-sm items-center gap-1.5">
                ...
                <FormControl>
                  <Input className="h-8" type="text" placeholder="3306" {...field} value={field.value} />
                </FormControl>
              </div>
              <FormMessage />
            </FormItem>
